...
These settings control label buffering to prevent text clipping at tile boundaries.
When enabled, point features (nodes, place centroids) are duplicated to adjacent tiles
when they appear within the buffer distance of a tile edge.

- `label_buffer_pixels` (default: 0 = disabled) - base buffer size in pixels (extent is 4096 pixels per tile). Points within this distance from the tile edge will be duplicated. Recommended values: 32-128 depending on font size.
- `label_buffer_start_zoom` (default: 0 = start from minzoom) - minimum zoom level at which buffering is applied. Lower zoom levels typically don't need buffering.
- `label_buffer_name_multiplier` (default: 0 = disabled) - additional buffer pixels per character in the name tag, so longer names get larger buffers. Example: 3 means a 10-character name gets 30 extra pixels.
- `label_buffer_max_width` (default: 0 = no maximum) - maximum buffer size in pixels, to prevent excessive buffering. Use with `label_buffer_name_multiplier` to cap the total buffer.

### Per-layer option

- `buffer` (default: true) - can be set per layer to disable buffering for specific layers, e.g. `{ "minzoom": 0, "maxzoom": 14, "buffer": false }`.

### Performance notes

- Buffering increases tile generation time and memory usage.
- Each buffered point can appear in up to 4 tiles.
- Use `label_buffer_start_zoom` to limit buffering to higher zooms only.
...
{
    "settings": {
        "label_buffer_pixels": 64,
        "label_buffer_start_zoom": 10,
        "label_buffer_name_multiplier": 3,
        "label_buffer_max_width": 200
    },
    "layers": {
        "place": { "minzoom": 0, "maxzoom": 14, "buffer": true },
        "poi":   { "minzoom": 12, "maxzoom": 14, "buffer": false }
    }
}
```
